...
| M7 | ✅ **MOB-7 — Statuts / temps** (`GET/PATCH api/time`) | M | **Livré 2026-06-08**. Timeline ack/read/go/onsite/terminate en `MOB_MISSION_STATE` opérationnelle (délégation `JobRepository`→`JobTimeRepository`). Projection statut fin→`ORD_MISSION.MIS_STATUS` **différée** (pas de transition de statut côté domaine Orders ; à cadrer ERP). Sirus : statu quo (MOB-16). |
...
| M11 | **MOB-11 — Conducteur** (`api/driver`) | S | Mappe sur `CRW_DRIVER_ASSIGNMENT` (ERP) en lecture/écriture. |
