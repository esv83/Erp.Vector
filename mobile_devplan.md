...
| ✅ **ACK / « Bien reçu »** (2026-07-05) | Acquittement d'une mission depuis la JobList (icône qui disparaît une fois acquittée). **Bug corrigé** : `PATCH /api/joblist` écrivait `MST_READ_AT` (jamais `MST_ACK_AT`) → `IsAck` restait `false`. `ClReadJobUseCase`→**`ClAckJobUseCase`** : pose `AckTime = Now` (`MST_ACK_AT`), **idempotent** (déjà acquitté = no-op, horodatage d'origine conservé). Renommage `ReadJob`→`AckJob` (service/interface/contrôleur). Chaîne inchangée : `JobTimeRepository.Save` upsert BD Mobile **+ projette `ackAt` vers Orders.Api** (`PUT missions/{id}/operational`, TRF-5) pour la régulation. UI = affichage seul (`IsAck` masque l'icône). Validé réel : `IsAck` false→PATCH 200→true, idempotence OK, projection sans erreur. **Reste côté Orders.Api** : persister/afficher l'ack (« bien reçu à HH:MM ») en liste des missions + historique. |
...
- **⚠️ ACK vs « Mission vue » (spec DMZ §10)** : la spec retient « Mission vue = Oui / **Mission acquittée = Non** ». Or MOB a livré l'acquittement explicite (`MST_ACK_AT`, `PATCH /api/joblist`). À arbitrer : conserver l'ack, ou le fusionner avec « Mission vue » (`MissionSeen`). Cf. §2bis.
