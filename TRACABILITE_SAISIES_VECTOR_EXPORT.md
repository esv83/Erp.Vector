...
| **T2** | **Le plan mutuelle dit l'inverse du code.** `MUTUELLE_CARD_devplan.md:101` annonce que la facturation tire le bloc mutuelle et son `imageUrl` — « c'est fait ». Le client HTTP existe, mais le bloc n'est pas désérialisé et aucune colonne ne le lit. À corriger d'un côté ou de l'autre. *(2026-09-13 : levé pour la mutuelle. BillingGateway déclare le bloc depuis `d048558` et a décidé qu'aucune colonne ne le lirait — `M7` ; le plan mutuelle est recalé.)* | [`MUTUELLE_CARD_devplan.md`](MUTUELLE_CARD_devplan.md) §2, §4 |
